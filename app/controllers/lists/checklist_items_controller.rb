module Lists
  class ChecklistItemsController < ApplicationController
    ...
    def create
      @checklist_item = @list.checklist_items.build(checklist_item_params)
    ...
    def reorder
      params[:item_ids].each_with_index do |id, index|
        @list.checklist_items.find(id).update(position: index)
      end
      render json: { success: true }
    end
    ...
    def set_checklist_item
      @checklist_item = @list.checklist_items.find(params[:id])
    end
